...
resource "proxmox_virtual_environment_network_linux_bridge" "kiwi_vmbr13" {
  node_name = "kiwi"
  name      = "vmbr13"
}
...
resource "proxmox_virtual_environment_vm" "crt" {
  name            = "crt"
  node_name       = "kiwi"
  on_boot         = true
  scsi_hardware   = "virtio-scsi-single"
  ...
  network_device {
    bridge = proxmox_virtual_environment_network_linux_bridge.kiwi_vmbr13.name
  }
  initialization {
    ip_config {
      ipv4 {
        address = "192.168.151.104/23"
        gateway = "192.168.150.1"
      }
    }
    user_account {
