...
resource "google_iam_workload_identity_pool_provider" "repo_github_actions" {
  ...
  attribute_condition                = "assertion.repository == \"${var.repo}\" || assertion.repository == \"${local.cicd_repo}\""
  attribute_mapping = {
...
resource "google_service_account_iam_member" "terraform_github_actions_selfhost_cicd_branch_workload_identity_user" {
  service_account_id = google_service_account.terraform_github_actions.id
  role               = "roles/iam.workloadIdentityUser"
  member             = "principal://iam.googleapis.com/projects/${var.project_number}/locations/global/workloadIdentityPools/${var.workload_identity_pool_id}/subject/repo:${local.cicd_repo}:ref:refs/heads/main"
}
